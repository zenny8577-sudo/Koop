...
const AddressBook: React.FC<AddressBookProps> = ({ user, onUpdate }) => {
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    const added = db.addAddress(user.id, { 
      ...newAddress, 
      isDefault: isFirstAddress 
    });
    
    if (added) {
      onUpdate(db.getUser(user.id));
  ...
  const removeAddress = (id: string) => {
    if (confirm('Wilt u dit adres definitief verwijderen?')) {
      ...
      const updated = db.updateUser(user.id, { addresses: filtered });
  ...
  const setAsDefault = (id: string) => {
    ...
    const updated = db.updateUser(user.id, { addresses: updatedAddresses });
    if (updated) onUpdate(updated);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {(user.addresses || []).length === 0 ? (
          <div className="md:col-span-2 py-20 bg-slate-50 rounded-[40px] border border-dashed border-slate-200 text-center space-y-4">
            ...
            <button onClick={() => setShowAddForm(true)} className="text-[#FF4F00] font-black uppercase text-xs tracking-widest underline">Voeg uw eerste adres toe</button>
          </div>
        ) : (
          (user.addresses || []).map(addr => (
            <div key={addr.id} className={`p-10 bg-white rounded-[40px] border transition-all duration-500 relative group overflow-hidden ${addr.isDefault ? 'border-[#FF4F00] shadow-2xl shadow-orange-500/5' : 'border-slate-100 hover:shadow-xl'}`}>
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => removeAddress(addr.id)} className="text-slate-200 hover:text-rose-500 transition-colors">
          ...
          <form onSubmit={handleAdd} className="relative bg-white w-full max-w-2xl p-12 lg:p-16 rounded-[60px] shadow-3xl overflow-y-auto max-h-[92vh]">
            <header className="flex justify-between items-start mb-12">
              <div className="space-y-4">
                ...
                <h2 className="text-4xl font-black uppercase tracking-tighter text-slate-950 leading-none">Adres <br /> Toevoegen.</h2>
              </div>
              <button type="button" onClick={() => setShowAddForm(false)} className="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center text-slate-400 hover:bg-slate-950 hover:text-white transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2 space-y-2">
                ...
                <input required placeholder="LABEL" value={newAddress.label} onChange={e => setNewAddress({...newAddress, label: e.target.value})} className="w-full bg-slate-50 border-none rounded-3xl px-8 py-5 text-sm font-bold focus:ring-4 focus:ring-orange-500/10 outline-none transition-all uppercase tracking-tight" />
              </div>
              
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-4">Voornaam</label>
                <input required placeholder="VOORNAAM" value={newAddress.firstName} onChange={e => setNewAddress({...newAddress, firstName: e.target.value})} className="w-full bg-slate-50 border-none rounded-3xl px-8 py-5 text-sm font-bold focus:ring-4 focus:ring-orange-500/10 outline-none transition-all" />
              </div>
              
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-4">Achternaam</label>
                <input required placeholder="ACHTERNAAM" value={newAddress.lastName} onChange={e => setNewAddress({...newAddress, lastName: e.target.value})} className="w-full bg-slate-50 border-none rounded-3xl px-8 py-5 text-sm font-bold focus:ring-4 focus:ring-orange-500/10 outline-none transition-all" />
              </div>

              <div className="md:col-span-2 space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-4">Straat & Huisnummer</label>
                <input required placeholder="BIJV. KEIZERSGRACHT 123" value={newAddress.street} onChange={e => setNewAddress({...newAddress, street: e.target.value})} className="w-full bg-slate-50 border-none rounded-3xl px-8 py-5 text-sm font-bold focus:ring-4 focus:ring-orange-500/10 outline-none transition-all" />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-4">Postcode</label>
                <input required placeholder="1234 AB" value={newAddress.zipCode} onChange={e => setNewAddress({...newAddress, zipCode: e.target.value})} className="w-full bg-slate-50 border-none rounded-3xl px-8 py-5 text-sm font-bold focus:ring-4 focus:ring-orange-500/10 outline-none transition-all" />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-4">Stad</label>
                <input required placeholder="AMSTERDAM" value={newAddress.city} onChange={e => setNewAddress({...newAddress, city: e.target.value})} className="w-full bg-slate-50 border-none rounded-3xl px-8 py-5 text-sm font-bold focus:ring-4 focus:ring-orange-500/10 outline-none transition-all" />
              </div>

              <div className="md:col-span-2 space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-4">Telefoonnummer</label>
                <input required type="tel" placeholder="+31 6 12345678" value={newAddress.phone} onChange={e => setNewAddress({...newAddress, phone: e.target.value})} className="w-full bg-slate-50 border-none rounded-3xl px-8 py-5 text-sm font-bold focus:ring-4 focus:ring-orange-500/10 outline-none transition-all" />
              </div>

              <div className="md:col-span-2 pt-6">
                <button type="submit" className="w-full py-8 bg-slate-950 text-white font-black rounded-full uppercase tracking-[0.3em] text-[11px] shadow-3xl hover:bg-[#FF4F00] transition-all transform hover:-translate-y-1">Adres Opslaan</button>
              </div>
            </div>
          </form>
        </div>
