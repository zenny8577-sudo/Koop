...
const SellRegistrationForm: React.FC<SellRegistrationFormProps> = ({ onSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    // Simulação de envio para o Supabase/CRM
    setTimeout(() => {
      setIsLoading(false);
      onSuccess();
    }, 1500);
      ...
      <div className="space-y-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <button 
            type="submit"
            disabled={isLoading}
            className="w-full py-7 bg-slate-900 text-white font-black rounded-3xl uppercase tracking-widest text-[12px] shadow-2xl hover:bg-[#FF4F00] transition-all"
          >
