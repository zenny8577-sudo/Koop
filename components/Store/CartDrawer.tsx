          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8">
            {items.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                ...
                <button onClick={onClose} className="text-[#FF4F00] font-black uppercase text-xs tracking-widest underline">Begin met shoppen</button>
              </div>
            ) : (
              items.map((item) => (
                <div key={item.product.id} className="flex gap-6 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center border border-slate-100 rounded-full overflow-hidden">
                        <button onClick={() => onUpdateQuantity(item.product.id, -1)} className="px-3 py-1 hover:bg-slate-50 text-slate-400 hover:text-slate-900">-</button>
                  ...
                  <button onClick={() => onRemove(item.product.id)} className="text-slate-200 hover:text-rose-500 transition-colors">
