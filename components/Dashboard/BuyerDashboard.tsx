  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase
        .from('reviews')
        .insert([{
          product_id: selectedTransaction.productId,
          user_id: user.id,
          user_name: user.firstName || user.email.split('@')[0],
          rating: rating,
          comment: comment,
      ...
      alert('Er is iets misgegaan bij het indienen van je review.');
    }
  };
                    ...
                    {transactions.map(t => {
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-12 py-8"><span className="text-sm font-black text-slate-900 uppercase tracking-tight">Product Title</span></td>
                          ...
                          <td className="px-12 py-8 text-center">
                            {hasReviewed ? (
                              ...
                              <button 
                                ...
                                className="text-[10px] font-black text-blue-600 hover:text-white hover:bg-blue-600 border border-blue-600 px-4 py-2 rounded-xl transition-all uppercase tracking-widest"
                              >
                                Review Schrijven
                              </button>
                            )}
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 animate-fadeIn">
          ...
          <form onSubmit={handleSubmitReview} className="relative bg-white w-full max-w-xl p-12 rounded-[60px] shadow-3xl space-y-10">
            <div className="space-y-4">
              ...
              <h2 className="text-4xl font-black text-slate-950 tracking-tighter uppercase leading-none">Review voor Product</h2>
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="flex gap-4 px-4">
                  {[1, 2, 3, 4, 5].map(s => (
                    <button 
                      ...
                      type="button" 
                      onClick={() => setRating(s)} 
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${rating >= s ? 'bg-amber-400 text-white shadow-lg shadow-amber-500/20' : 'bg-slate-50 text-slate-300'}`}
                    >
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  required 
                  placeholder="Wat vond je van het product en de curatie?" 
                  className="w-full bg-slate-50 border-none rounded-3xl px-8 py-6 text-sm font-bold outline-none h-40 focus:ring-4 focus:ring-blue-500/10 transition-all" 
                  value={comment} 
            ...
            <div className="flex gap-4">
              <button 
                ...
                className="flex-1 py-6 bg-slate-50 text-slate-400 font-black rounded-3xl uppercase tracking-widest text-[11px] hover:bg-slate-100 transition-all"
              >
                Annuleren
              </button>
              <button 
                type="submit" 
                className="flex-1 py-6 bg-slate-950 text-white font-black rounded-3xl uppercase tracking-widest text-[11px] shadow-2xl hover:bg-blue-600 transition-all"
              >
                Verzenden
              </button>
            </div>
