...
const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  // Tenta pegar a segunda imagem para o hover, se existir
  const hoverImage = product.gallery && product.gallery.length > 1 ? product.gallery[1] : product.image;

  const handleWishlist = (e: React.MouseEvent) => {
    e.stopPropagation();
    onToggleWishlist?.(product.id);
    AnalyticsService.trackEvent('wishlist_toggle', { productId: product.id });
  };

  const handleAddToCartClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    onAddToCart?.(product);
    AnalyticsService.trackEvent('add_to_cart', { productId: product.id });
  };
  ...
  return (
    <div className="group flex flex-col h-full bg-transparent select-none">
      <div 
        onClick={() => onClick?.(product)}
        className="relative aspect-[4/5] bg-slate-50 dark:bg-neutral-900 rounded-[40px] overflow-hidden shadow-sm border border-slate-100/50 dark:border-white/5 transition-all duration-500 hover:shadow-2xl hover:border-slate-200 cursor-pointer"
      >
        {/* Main Image */}
        <img 
          src={product.image} 
          alt={product.title}
          className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-500 ${hoverImage !== product.image ? 'group-hover:opacity-0' : ''}`}
        />
        
        {/* Hover Image (Second in Gallery) */}
        ...
        {isVerified && (
          <div className="absolute top-5 left-5 z-10">
            <div className="bg-white/90 dark:bg-black/80 backdrop-blur px-3 py-1.5 rounded-xl shadow-lg flex items-center gap-2 border border-white/50">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-slate-950/40 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-500 flex gap-2">
          <button 
            ...
            className="flex-1 py-4 bg-white text-slate-900 font-black text-[10px] uppercase tracking-widest rounded-2xl shadow-xl hover:bg-slate-950 hover:text-white transition-all transform active:scale-95"
          >
            Details
          </button>
          <button 
      ...
      <div className="pt-6 flex justify-between items-start">
        <div className="space-y-1 group-hover:translate-x-1 transition-transform">
          <h3 
            onClick={() => onClick?.(product)} 
            className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tighter cursor-pointer hover:text-[#FF4F00] transition-colors line-clamp-1 pr-4"
          >
            {product.title}
          </h3>
